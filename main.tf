...
resource "null_resource" "immich_setup" {
  ...
  provisioner "file" {
    content = <<-EOT
      ...
      export BACKBLAZE_KEY_ID='${var.backblaze_application_key_id}'
      export BACKBLAZE_KEY='${var.backblaze_application_key}'
      export BACKBLAZE_BUCKET='${var.backblaze_bucket_name}'
      export BACKBLAZE_REGION='${var.backblaze_region}'
